...
if [ -d "$ROOTDIRSRAW" ] ; then
    ROOTDIRS=`cygpath --path --mixed "$ROOTDIRSRAW" 2>/dev/null`
else
    ROOTDIRS=$ROOTDIRSRAW
fi

if [ ! -f "$CLASSPATH" ] ; then
    die "ERROR: JAVA_HOME is not defined correctly in system variables or gradle wrapper jar file is missing"
fi
